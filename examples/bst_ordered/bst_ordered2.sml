...
(* The only difference between this function and the function written in
  * bst_ordered1.sml is between leq and geq. In the general case, where leq and
  ...
  * bst_ordered1.sml (this can be seen by considering the behavior when
  ...
  * bst_ordered1.sml, int_bst_ordered is equivalent between the two files. *) 

fun bst_ordered geq T =
  let
      ...
      | bst_ordered_helper (Node (L, x, R)) =
          let
            ...
            val left_tree_smaller =
             case L_max_opt of
                  NONE => true
                | SOME(L_max) => geq(x, L_max)
                    
            val right_tree_bigger =
              case R_min_opt of
                   NONE => true
                 | SOME(R_min) => geq(R_min, x)
